...
├── AGENTS.trellis.md
├── registry/
│   ├── skills.yaml    # repo-managed / vendored skill 清单
│   └── plugins.yaml   # 插件托管 skill 迁移记录
├── scripts/
│   ├── audit-global-skills.sh
│   ├── link-global-skills.sh
...
| [`ui-ux-pro-max`](skills/design/ui-ux-pro-max/SKILL.md) | 跨 Web / Mobile 的 UI/UX 设计规则、视觉系统、交互质量和前端体验检查。 |
...
本仓库是本机 repo-managed 与 vendored skill 的 source of truth。`~/.codex/skills` 与 `~/.claude/skills` 只作为全局入口，通过 symlink 指向本仓库。

| 文件 | 作用 |
| --- | --- |
| `registry/skills.yaml` | 记录 repo-managed / vendored skill、全局入口、同步来源。 |
| `registry/plugins.yaml` | 记录插件托管 skill 的来源和更新策略，迁移设备时用作安装清单。 |
| `scripts/audit-global-skills.sh` | 只读检查 Codex / Claude 全局入口是否指向本仓库。 |
| `scripts/link-global-skills.sh` | 将 registry 中启用的 skill 链接到 Codex / Claude 全局目录，并备份原有目录。 |
| `scripts/sync-vendored-skills.sh` | 从外部来源同步 vendored skill 到本仓库。 |

常用命令：

```bash
scripts/sync-vendored-skills.sh ui-ux-pro-max trellis-start
scripts/link-global-skills.sh
scripts/audit-global-skills.sh
```
...
- Trellis 系列：来自本机 `/Users/zhici/per-pro/personal-kb/.agents/skills/trellis-*`
