...
### Choose one research owner

For each independent research topic, either dispatch `trellis-research` or run Matt `research` from the main session. Do not use both for the same topic.
...
> **Codex dispatch contract**: a `trellis-research` prompt starts with `Active task: <task path>`. The child reads the task from that path and must not spawn descendants. Use Matt `research` only from the main session because its native flow may delegate further.
...
When open questions are resolved, confirm complete requirements with a structured summary. Approval closes brainstorming but does not start implementation yet.
...
Does this look correct? If yes, I'll run the final plan review before implementation.
```
...
After brainstorm completes (Step 8 confirmation approved), run `grill-me` as an adversarial review gate. Resolve every blocking finding in the task artifacts, obtain confirmation on the revised plan, then continue to context curation and activation:
...
Brainstorm
  ...
  ↓
Matt plan review
  grill-me: stress-test scope, contracts, failure states, and acceptance evidence
  → update task artifacts → resolve blocking findings → user confirms
  ↓
